...
|| **Completed Tasks** | 43 (T-001 through T-008, T-009 + subtasks, T-010, T-011, T-014 + subtasks a–c, T-015, T-015-a, T-016, T-021-a/b/c, T-021, T-022, T-023, T-024-a/b/c, T-029 + subtasks, T-030 + subtasks a–f) |
...
| `T-033` | **Welcome Rebrand — Cinematic Onboarding** — complete replacement of existing welcome overlay with song-driven, cinematic **2-minute** intro experience | `PENDING` | **User directive.** Song: "We Do What We Want (Edit)". Audio-reactive visuals, beat-synced text effects, feature zoom demos with popups, agent persona copy. Full rewrite of `welcome.js` + `welcome.css`. |
|| `T-033-a` | Song preloading & verification — `welcome.js` must verify audio file exists and is fully buffered before showing START button. Display loading spinner until `canplaythrough` event fires. | **AUDIO DELIVERED** 🎵 | ✅ File: `public/assets/audio/we-do-what-we-want-edit.mp3` (2.9MB, 64kbps, 44.1kHz). ⚠️ **DO NOT commit to GitHub** — copyrighted material. Added to `.gitignore`. Fallback (file missing): show "Upload your MP3" placeholder and disable START. |
|| `T-033-b` | Welcome screen phase 1 — dimmed dark overlay, center prompt: "What's your name?" + text input + glowing ENTER button. No app content visible behind. Background: near-black `#020202` with subtle vignette. | `PENDING` | Input focused automatically. ENTER triggers Phase 2. Name saved to `localStorage` (`ncc-welcome-name`). |
|| `T-033-c` | Phase 2 — dark cinematic **beat-reactive** sequence (0:00–0:22) — screen goes fully black, song starts playing, text phrases **flash/pop on every strong beat** using Web Audio API beat detection or hardcoded bpm timestamps. Phrases: "I DON'T SLEEP.", "I DON'T EAT.", "I WORK 24/7.", "Welcome to the future.", "The future is here.", "Are you ready?", "Let's build something extraordinary.", "Your personal command center awaits." | `PENDING` | Beat-reactive engine: `welcome.js` uses Web Audio `AnalyserNode` to detect bass frequencies and trigger CSS `scale(1.05)` + `brightness(1.2)` pulse on text. Each phrase is a DOM element with `animation: beatPulse 0.15s ease-out`. Hardcoded BPM fallback (128 BPM) if Web Audio unavailable. Font: bold condensed sans-serif, electric arc-blue glow `#39d0f2`, letter-spacing animates `0px → 4px → 0px` on each beat. |
|| `T-033-d` | Phase 3 — **feature showcase montage** (0:22–1:45) — screen brightens, **camera zooms into each app tile** with realistic demo popups/tooltips appearing next to them as if the user is actually using the app. Each zoom transition is **timed to a beat hit** (beat-synced fly-in). Features zoomed in sequence: Calendar (demo event being created), Notes (text auto-summarizing with AI sparkle), To-Do (tasks reordering by priority), Weather (live tile pulsing), IT Hub (system pulse heartbeat), Phone Bridge (SMS thread with "Sick? I'll text and email who needs to know"), Work Simulator (fake terminal scrolling), Arcade (Snake game briefly plays), Backup (shield encryption animation). Each zoom lasts ~8–10 seconds with demo popup annotations explaining the feature. **Beat-reactive background:** subtle particle burst + screen flash on every kick drum. | `PENDING` | CSS `transform: scale()` zoom into each app container. Demo popups are `position: absolute` overlays with arrow pointers, containing GIF or CSS-animated mockups. NOT a video — pure DOM choreography. `requestAnimationFrame` loop at 60fps. Beat detection drives `scale(1.0 → 1.02 → 1.0)` micro-jitter on the active tile for every bass hit. |
|| `T-033-e` | Phase 3 **detail scripting & copy** — exact lines for each showcase beat, written to **wow the user** with agent confidence. JSON-driven copy array: (1) "I DON'T SLEEP. I DON'T EAT. I WORK 24/7 FOR YOU." (2) "Sick? Just tell me — I'll put in a good word for you. I'll text. I'll email. Who needs to know?" (3) "Not sick and just wanna call off work? No problem. That stays with us. I'll call off work for you." (4) "Your calendar — optimized by AI. I know before you do." (5) "Your tasks — auto-prioritized. I sort the noise so you don't have to." (6) "Your notes — summarized in seconds. Walls of text? Done." (7) "Your system — monitored 24/7. I see everything. I fix everything." (8) "Your data — backed up and encrypted. Even I can't peek." (9) "Your games — right here when you need a break. High score's waiting." Each line appears **centered, large, bold** with icon/tile animating behind it. Text **pops on beat** with `scale(1.08)` + `text-shadow` burst. | `PENDING` | Configurable JSON object in `welcome.js`: `{ beatTime: seconds, copy: "...", tile: "calendar", icon: "📅", effect: "pulse|flash|zoom" }`. Agent can reorder or edit without touching JS. |
|| `T-033-f` | Phase 4 — landing (1:45–2:00) — song begins slow fade out over 15 seconds. Screen dims back to black for 1 second, then dashboard fades in with chosen theme active. Audio volume ramps down smoothly via `audio.volume` transition. If name was entered, header shows "Welcome back, {name}". First-time flag `ncc-welcome-shown` set to true so intro only plays once. | `PENDING` | Smooth transition: opacity 0→1 over 1.5s, audio `volume 1.0 → 0.0` linear over 15s. No jarring cut. Replay button available in Settings → "Replay Welcome Intro". |
|| `T-033-g` | Audio sync engine — `welcome.js` must track `audio.currentTime` and trigger phase transitions at exact timestamps (0:00 start, 0:22 cut to showcase, 1:45 start fade, 2:00 landing). Use `requestAnimationFrame` loop checking `audio.currentTime` against phase thresholds. Beat detection subsystem: `AnalyserNode` with FFT size 256, monitors bass frequencies (20–150Hz), triggers text pulse / screen flash / particle burst on threshold exceed. | `PENDING` | Prevents drift. Also handle `audio.ended` as fallback to force Phase 4. Mute/unmute respects `prefers-reduced-motion` (skip intro entirely if set). Beat detection runs independently of phase engine so text effects continue during any phase. |
