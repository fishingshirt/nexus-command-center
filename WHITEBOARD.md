...
| `T-033` | **Welcome Rebrand — Cinematic Onboarding** — complete replacement of existing welcome overlay with song-driven, cinematic 90-second intro experience | `PENDING` | **User directive.** Song: "We Do What We Want (Edit)". Full rewrite of `welcome.js` + `welcome.css`. |
...
|| `T-033-c` | Phase 2 — dark cinematic sequence (0:00–0:22) — screen goes fully black, song starts playing, text phrases fade in/out sequentially: "Welcome to the future.", "The future is here.", "Are you ready?", "Let's build something extraordinary.", "Your personal command center awaits." | `PENDING` | Phrases timed to beat drops. CSS keyframe animations: fade-in, hold 2–3s, fade-out. Font: bold sans-serif, electric arc-blue glow (`#39d0f2`), subtle letter-spacing animation. |
|| `T-033-d` | Phase 3 — showcase montage (0:22–1:30) — screen brightens, fast-cut/zoom-through demo of dashboard capabilities. Animated sequences showing: Calendar auto-scheduling, Notes AI summarization, To-Do smart prioritization, Weather live tile, IT Hub system pulse, Phone Bridge "Sick? I'll text and email who needs to know", Work Simulator "Not sick and wanna call off work? No problem — that stays with us and I'll call off work for you", Arcade games flash, Backup shield icon | `PENDING` | Each capability shown as a stylized "card fly-in" or "tile zoom" animation. Not a video — built from DOM elements with CSS transforms + JS choreography. Total duration: ~68 seconds. Synced to song tempo (upbeat). |
|| `T-033-e` | Phase 3 detail scripting — exact copy for each showcase beat: (1) "Sick? Just tell me — I'll put in a good word for you, text and email who needs to know." (2) "Not sick and just wanna call off work? No problem — that stays with us and I'll call off work for you." (3) "Your calendar, optimized by AI." (4) "Your tasks, auto-prioritized." (5) "Your notes, summarized in seconds." (6) "Your system, monitored 24/7." (7) "Your data, backed up and encrypted." | `PENDING` | Each line appears as centered text with matching icon/tile animating behind it. Build as configurable JSON array so copy can be edited without touching JS. |
|| `T-033-f` | Phase 4 — landing (1:30) — everything dims back to black for 1 second, then dashboard fades in with chosen theme active. If name was entered, header shows "Welcome back, {name}". First-time flag `ncc-welcome-shown` set to true so intro only plays once. | `PENDING` | Smooth transition: opacity 0→1 over 1.5s. No jarring cut. Replay button available in Settings → "Replay Welcome Intro". |
|| `T-033-g` | Audio sync engine — `welcome.js` must track `audio.currentTime` and trigger phase transitions at exact timestamps (0:00 start, 0:22 cut to showcase, 1:30 fade to dashboard). Use `requestAnimationFrame` loop checking `audio.currentTime` against phase thresholds. | `PENDING` | Prevents drift. Also handle `audio.ended` as fallback to force Phase 4. Mute/unmute respects `prefers-reduced-motion` (skip intro entirely if set). |
