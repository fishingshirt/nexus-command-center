...
|| `T-033-a` | Song preloading & verification — `welcome.js` must verify audio file exists and is fully buffered before showing START button. Display loading spinner until `canplaythrough` event fires. | `PENDING` | Path: `public/assets/audio/we-do-what-we-want-edit.mp3`. Fallback: if file missing, show "Upload your MP3" placeholder and disable START until user provides file. |
